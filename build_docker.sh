...
if [ $? -eq 0 ]; then
   if [ "$GOOGLE_MAP_JS_API_KEY" -ne "" ]; then
      ...
      echo for which 4xAPI needs to be assigned to the key.
      echo if you have the key, enter it now>
   ...
   if [ "$GOOGLE_MAP_JS_API_KEY" == "" ]; then
      ...
      sed -i "s/ENV GOOGLE_MAP_JS_API_KEY -/ENV GOOGLE_MAP_JS_API_KEY ${ENV GOOGLE_MAP_JS_API_KEY}/g" Docker/dockerfile
   fi
fi
grep "GOOGLE_GEOCODE_API_KEY -" Docker/dockerfile > /dev/null
if [ $? -eq 0 ]; then
   if [ "$GOOGLE_GEOCODE_API_KEY" -ne "" ]; then
      echo found the environment variable GOOGLE_GEOCODE_API_KEY
   elif [ "$GOOGLE_MAP_JS_API_KEY" -ne "" ]; then
      echo found the environment variable GOOGLE_MAP_JS_API_KEY
   ...
   if [ "$GOOGLE_GEOCODE_API_KEY" == "" ]; then
      ...
      sed -i "s/GOOGLE_GEOCODE_API_KEY -/GOOGLE_GEOCODE_API_KEY ${GOOGLE_GEOCODE_API_KEY}/g" Docker/dockerfile
   fi
fi
